...
def extract(roi0_img, save_debug=False):
    ...
    if save_debug:
        ...
        output_dir = 'ROI_Menu'
        os.makedirs(output_dir, exist_ok=True)
        # Use first 4 letters of filename if available, else 'menu'
        base = getattr(roi0_img, 'filename', None)
        if base:
            prefix = os.path.splitext(os.path.basename(base))[0][:4]
        else:
            prefix = 'menu'
        menu_path = os.path.join(output_dir, f'{prefix}_{now}.png')
        cv2.imwrite(menu_path, menu)
        ocr_txt_path = os.path.join(output_dir, f'{prefix}_{now}.txt')
        with open(ocr_txt_path, 'w') as f:
            f.write(ocr_text)
    return {
        'roi_id': 'ROI_Menu',
